...
Good luck, Memory Master!
    ...
    def start_game(self, level):
        ...
        # Calculate optimal card size based on window dimensions and grid
        self.root.update_idletasks() # Ensure window dimensions are up to date
        available_width = self.root.winfo_width() * 0.8 # Roughly 80% of window width for cards
        available_height = self.root.winfo_height() * 0.6 # Roughly 60% of window height for cards (leaving space for header/footer)

        # Consider padding and border around each card
        horizontal_space_per_card = (self.grid_cols * (4 * 2 + 3 * 2)) # padx*2 + bd*2 for each card
        vertical_space_per_card = (self.grid_rows * (4 * 2 + 3 * 2)) # pady*2 + bd*2 for each card

        card_width = (available_width - horizontal_space_per_card) / self.grid_cols
        card_height = (available_height - vertical_space_per_card) / self.grid_rows

        # Choose the smaller dimension to maintain aspect ratio and fit
        self.card_size = (int(min(card_width, card_height)), int(min(card_width, card_height)))

        # Ensure a minimum card size to prevent tiny cards
        min_card_dim = 70 # pixels
        self.card_size = (max(self.card_size[0], min_card_dim), max(self.card_size[1], min_card_dim))
